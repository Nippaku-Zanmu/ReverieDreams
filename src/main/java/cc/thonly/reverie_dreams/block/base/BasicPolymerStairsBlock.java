...
@Getter
public class BasicPolymerStairsBlock extends StairsBlock implements FactoryBlock, IdentifierGetter {
    ...
    public BasicPolymerStairsBlock(String path, BlockState baseBlockState, Settings settings) {
        this(Touhou.id(path), baseBlockState, settings);
    }

    @Override
    public BlockState getPolymerBlockState(BlockState state, PacketContext context) {
        return template.getDefaultState().with(FACING, state.get(FACING)).with(SHAPE, state.get(SHAPE)).with(HALF, state.get(HALF)).with(WATERLOGGED, state.get(WATERLOGGED));
    }

    @Override
    public @Nullable ElementHolder createElementHolder(ServerWorld world, BlockPos pos, BlockState initialBlockState) {
        return new Model(initialBlockState, this.getIdentifier().getPath());
    }
    ...
    public static final class Model extends BlockModel {
        ...
        public Model(BlockState state, String path) {
            MODEL_STRAIGHT = ItemDisplayElementUtil.getModel(Touhou.id("block/" + path));
            MODEL_CORNER_INNER = ItemDisplayElementUtil.getModel(Touhou.id("block/" + path + "_inner"));
            MODEL_CORNER_OUTER = ItemDisplayElementUtil.getModel(Touhou.id("block/" + path + "_outer"));
            init(state);
        }
