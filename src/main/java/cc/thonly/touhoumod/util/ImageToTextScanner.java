...
public class ImageToTextScanner {
    ...
    public static void registerBuffer() {
        CompletableFuture.runAsync(() -> {
            BufferedImage iconBuffer = ImageToTextScanner.getInstance().loadImageFromJar("/assets/" + Touhou.MOD_ID + "/icon.png");
            List<Text> text = ImageToTextScanner.getInstance().renderImageToText(iconBuffer, 16, 16);

        });
    }

    public static ImageToTextScanner getInstance() {
        if (INSTANCE == null) {
