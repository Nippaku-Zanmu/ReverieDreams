...
public class DanmakuEntity extends PersistentProjectileEntity implements PolymerEntity {
    ...
    public DanmakuEntity(LivingEntity livingEntity, ItemStack stack, Hand hand, Item item) {
        this(livingEntity, stack, hand, item, livingEntity.getPitch(), livingEntity.getYaw(), 1.5F, 0, 0.4f);
    }

    public DanmakuEntity(Entity livingEntity, ItemStack stack, Hand hand, Item item, float pitch, float yaw, float speed, float divergence, float offsetDist, boolean tile) {
        this(livingEntity, stack, hand, item, pitch, yaw, speed, divergence, offsetDist);
        this.tile = tile;
    }

    public DanmakuEntity(Entity livingEntity, ItemStack stack, Hand hand, Item item, float pitch, float yaw, float speed, float divergence, float offsetDist) {
        super(ModEntities.DANMAKU_ENTITY_TYPE,
                ...
                stack.copy(),
                stack.copy());
        this.setupPitch = livingEntity.getPitch();
        this.setupYaw = livingEntity.getYaw();

        double offsetX = -Math.sin(Math.toRadians(yaw)) * offsetDist;
        double offsetZ = Math.cos(Math.toRadians(yaw)) * offsetDist;

        double newX = livingEntity.getX() + offsetX;
        double newY = livingEntity.getY() + livingEntity.getStandingEyeHeight();
        double newZ = livingEntity.getZ() + offsetZ;
